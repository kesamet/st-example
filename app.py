...
@st.experimental_memo
def _cast_gdf(df, fill_color):
    _df = df.copy()

    _df["geometry"] = _df["h3"].apply(h3_to_polygon)
    ...
    # _df["coordinates"] = _df["geometry"].apply(lambda x: list(mapping(x)["coordinates"][0]))
...
def page_analysis():
    select_version = st.sidebar.selectbox("Version", ["Autoencoder", "Binary"])
    if select_version == "Autoencoder":
        v = 13
    else:
        v = 10
    embeddings = load_embeddings(f"data/embeddings_h3_9_v{v}.npy")
    df = load_csv("data/h3_city.csv")

    select_city = st.sidebar.selectbox("Select city", list(CITIES.keys()))
    select_regions = df["city"] == select_city
    fill_color = pca_scale(embeddings[select_regions])

    centre, df0 = _cast_gdf(df[select_regions], fill_color)
    add_map(centre, df0)
